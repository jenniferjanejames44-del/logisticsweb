    ...
    <section ref={ref} className="section-padding bg-background">
      <div className="section-container">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14 lg:mb-20">
          ...
          <h2
            className={`text-primary mb-5 transition-all duration-700 delay-100 ${
              isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
            }`}
          >
          ...
          <p
            className={`text-foreground/80 text-lg md:text-xl font-medium leading-relaxed transition-all duration-700 delay-200 ${
              isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
            }`}
          >
        ...
        <div className="grid sm:grid-cols-2 gap-6 lg:gap-8">
          {items.map((item, index) => (
            ...
            >
              ...
              <div
                className={`w-14 h-14 sm:w-16 sm:h-16 ${item.color} rounded-xl sm:rounded-2xl flex items-center justify-center mb-5 md:mb-6 group-hover:scale-110 shadow-md transition-all duration-300`}
              >
                <item.icon size={26} className={`${item.iconColor}`} />
              </div>
              <h3 className="text-xl md:text-2xl font-bold text-foreground mb-3 sm:mb-4 group-hover:text-accent transition-colors duration-300">
                {item.title}
              </h3>
              <p className="text-foreground/70 text-base font-medium leading-relaxed">
                {item.description}
              </p>
            </div>
          ))}
