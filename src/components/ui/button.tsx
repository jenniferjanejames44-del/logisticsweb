...
const buttonVariants = cva(
  "font-display inline-flex w-auto select-none items-center justify-center gap-2 rounded-lg text-center text-[1rem] font-semibold leading-tight ring-offset-background transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/10 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none disabled:transform-none [&_svg]:pointer-events-none [&_svg]:size-[18px] [&_svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        /* Mobile-first: 44px on mobile → 48px on sm+ (global design system) */
        default: "h-11 px-5 py-2.5 text-[15px] sm:h-12 sm:px-6 sm:text-base",
        compact: "h-10 px-4 py-2 text-[14px] font-semibold sm:h-11 sm:px-5",
        sm: "h-10 px-4 py-2 text-[14px] font-semibold whitespace-nowrap sm:h-11 sm:px-5",
        lg: "h-11 px-5 py-2.5 text-[15px] sm:h-12 sm:px-6 sm:text-base",
        xl: "h-12 px-6 py-3 text-base font-semibold sm:h-[52px] sm:px-7",
        icon: "h-11 w-11 sm:h-12 sm:w-12",
        iconSm: "h-9 w-9",
        nav: "h-10 px-4 py-2 text-[14px] font-semibold whitespace-nowrap sm:h-11 sm:px-5 sm:text-[15px]",
        dash: "h-11 px-5 py-2.5 text-[15px] font-semibold whitespace-nowrap sm:h-12 sm:px-6",
        dashSm: "h-10 px-4 py-2 text-[14px] font-semibold whitespace-nowrap sm:h-11 sm:px-5",
      },
    },
    defaultVariants: {
      variant: "default",
