...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!amount || !callback_url) {
      return new Response(
        JSON.stringify({ error: "amount and callback_url are required" }),
    ...
    const reference = `WALLET-${userId.substring(0, 8)}-${Date.now()}`;
    ...
    if (!paystackRes.ok || !paystackData.status) {
      console.error("Paystack wallet topup init failed:", paystackData);
